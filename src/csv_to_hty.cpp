...
void convert_from_csv_to_hty(const std::string& csv_file_path, const std::string& hty_file_path) {
    ...
    while (std::getline(csv_file, line)) {
        ...
        while (std::getline(iss, value, ',')) {
            ...
            // Store data as int (we'll interpret float and string later if needed)
            if (columns[col_index].type == "float") {
                ...
                columns[col_index].data.push_back(0);
            }
            col_index++;
        }
        num_rows++;
    ...
    metadata["num_groups"] = 1;  // We'll put all columns in one group for simplicity
    json group;
